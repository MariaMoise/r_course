...
Конечно, можно не полагаться на алфавит и самому указать любую базовую категорию. по умолчанию y=L=1 y=H=0 LH - меняются по алфавиту

Создадим другой набор данных с альтернативной базовой категорией:

```{r}
educ_fct_rel <- mutate(educ_fct, y = fct_relevel(y, 'L'))
```

3. Разбиваем выборку на две части: 80% в обучающей и 20% в тестовой (80% y=1 и у=0 идет в обучающую 20% у=1 y=0 идет в тестовую) 
...
head(educ_pred)
# номер наблюдения в тестовой выборке
...
```
reference - истина(на основе предыдущих показаний, на основе теста) prediction - прогноз
Accuracy - % правильно предсказанных стандартный интервал для него

sensitivity -TP/(TP+FN) верно предсказанные y = H true positive false negative 
true negative - точно предсказанные у=L
false positive - неверно предсказанных у=Н
false negative - неверно предсказанные у=L

Specificity - доля верно предсказанных L
...
y ~ . - строим регерессию у от всех остальных переменных
методы оценки важности
1) permutation - случайное переставление регрессоров в прогнозе 
2) impurity - находим переменную из-за которой больше всего падает однородность
```{r}
set.seed(777)
